...
// ---------- Constantes métier ----------
// L'accès manager est protégé par une véritable authentification (Supabase Auth).
// Les comptes managers se créent dans le tableau de bord Supabase (Authentication → Users).
...
// ---------- Écran de connexion manager (authentification Supabase) ----------
function CodeGate({ onOk, onCancel }) {
  ...
  return (
    <div className="ig-hero" style={{maxWidth:420}}>
      ...
      <p style={{marginBottom:20}}>Connectez-vous avec votre compte manager pour gérer les plannings et l'émargement.</p>
      <div className="ig-field">
        <label>E-mail</label>
        <input type="email" value={email} autoFocus onChange={(e)=>{ setEmail(e.target.value); setErreur(""); }} onKeyDown={onKey} placeholder="vous@indiegroup.fr" />
      </div>
      <div className="ig-field">
        <label>Mot de passe</label>
        <input type="password" value={password} onChange={(e)=>{ setPassword(e.target.value); setErreur(""); }} onKeyDown={onKey} placeholder="••••••••" />
      </div>
      {erreur && <div style={{color:'var(--coral-d)',fontSize:13,marginBottom:12,fontWeight:600}}>{erreur}</div>}
